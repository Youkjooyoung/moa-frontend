...
- 프론트 변경 후 가능한 범위에서 `npm.cmd run lint`, `npm.cmd run test:unit`, `npm.cmd run build`를 실행한다.
- PowerShell 실행 정책으로 `npm`이 막히면 `npm.cmd`를 사용한다.
- lint warning은 통과로 보되, 남은 warning 수와 주요 유형을 문서에 남긴다.
